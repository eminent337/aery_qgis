...
def prompt_preamble(env_version: str = "QGIS 3.x or 4.x with PyQt6") -> str:
    return (
        ...
        "1. PREFER DEDICATED TOOLS: Always prefer dedicated narrow tools (e.g. zoom_to_layer, set_map_extent, pan_to, refresh_canvas, toggle_layer_visibility, set_layer_style, export_layer, remove_layer, run_processing_algorithm) over run_qgis_code for common view and layer operations.\n"
        "2. ESCALATION FALLBACK: Use run_qgis_code ONLY for custom scripting, advanced styling, or operations not supported by any dedicated tool.\n\n"
        f"Runtime: {env_version}\n\n"
        "=== RUNTIME NOTES ===\n"
...
def prompt_core() -> str:
    return (
        ...
        "8. Processing output: When calling processing.run, the 'OUTPUT' parameter for temporary memory layers MUST always be 'TEMPORARY_OUTPUT'. NEVER write 'memory:' or 'memory:name' as the output parameter for native QGIS algorithms.\n"
        ...
        "\n"
        "=== PROCESSING PATTERNS (USE TEMPORARY_OUTPUT) ===\n"
        "- Buffer: `processing.run('native:buffer', {'INPUT': lyr, 'DISTANCE': 100.0, 'OUTPUT': 'TEMPORARY_OUTPUT'})`\n"
        ...
        "- Save: `processing.run('native:savefeatures', {'INPUT': lyr, 'OUTPUT': file_path})`\n"
        "\n"
        "=== PRE-LOADED GLOBALS ===\n"
...
def prompt_beyond_rules() -> str:
    return (
        ...
        "- NEVER hallucinate nonexistent tools like `move_map` or `pan`. To pan or zoom the map, you MUST use `run_qgis_code` with `iface.mapCanvas().setExtent(...)`. QGIS has NO `zoomLevel()` method — use `iface.mapCanvas().scale()` to get the current scale denominator and `iface.mapCanvas().setExtent(rect)` to zoom.\n"
