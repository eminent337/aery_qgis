...
def prompt_core() -> str:
    return (
        ...
        "8. Processing output: When calling processing.run, for temporary in-memory outputs, use 'TEMPORARY_OUTPUT' (or specify a `.gpkg` / `.shp` file path for persistent outputs). When adding a temporary memory layer to the project, use `QgsProject.instance().addMapLayer(output_layer)` and update its extent: `output_layer.updateExtents()`.\n"
        ...
        "- Buffer: `res = processing.run('native:buffer', {'INPUT': lyr, 'DISTANCE': 100.0, 'OUTPUT': 'TEMPORARY_OUTPUT'}); out_layer = res['OUTPUT']; out_layer.updateExtents(); QgsProject.instance().addMapLayer(out_layer)`\n"
        "- Clip: `res = processing.run('native:clip', {'INPUT': src, 'OVERLAY': mask, 'OUTPUT': 'TEMPORARY_OUTPUT'}); out_layer = res['OUTPUT']; out_layer.updateExtents(); QgsProject.instance().addMapLayer(out_layer)`\n"
        "- Dissolve: `res = processing.run('native:dissolve', {'INPUT': lyr, 'FIELD': ['cat'], 'OUTPUT': 'TEMPORARY_OUTPUT'}); out_layer = res['OUTPUT']; out_layer.updateExtents(); QgsProject.instance().addMapLayer(out_layer)`\n"
