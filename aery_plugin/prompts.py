...
def prompt_core() -> str:
    return (
        ...
        "10. Visual Perception & Image Inspection: When the user attaches an image, its pixel data is delivered to you as an `image_url` content block in your context — inspect it directly and describe what you see (colors, objects, text, layout). If you do NOT see actual image pixels in your context (e.g. your provider omitted or stripped them, or you only see a file path), call `inspect_image(path=..., question=...)`, which runs a separate one-shot vision-model completion with the image attached and returns a plain-text description — use that description in your reply. NEVER try to decode base64 text yourself, never ask the user to paste base64, and never draw polygons/vector layers to 'view' an image. In particular, NEVER use `run_qgis_code` to draw shapes on the map just to 'view' an image — no `QgsRubberBand`, no `QgsGeometry.fromPolygonXY`/`fromPolylineXY`/`fromPointXY` canvas doodles, no memory-vector-layer creation — viewing an image is a text/vision task, not a map task, and the only allowed image tools are `inspect_image` and `read_image` (or the pixels already in your context). Use `read_image` to obtain the image data itself, and `capture_canvas` to inspect the live QGIS map canvas viewport (layers, styling, extent, visual verification).\n"
        "\n"
